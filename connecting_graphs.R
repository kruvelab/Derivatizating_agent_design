...
connect_substructures = function(SMILES1, SMILES2) {
  ...
  graph2 = set.vertex.attribute(graph2_list$graph, "name", value=as.character((length(graph1_list$atoms)+1):(length(graph1_list$atoms)+length(graph2_list$atoms))))
  ...
  available_connections_first_SMILES = atoms_available_for_connection(graph1_list$names_of_atoms, graph1_list$matrix_with_bond_count)
  available_connections_second_SMILES = atoms_available_for_connection(graph2_list$names_of_atoms, graph2_list$matrix_with_bond_count)
